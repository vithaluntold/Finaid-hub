...
>(
  (
    ...
    // Flatten and count children that are SelectItems
    const items = React.Children.toArray(children).filter(
      (child: any) => child?.type?.displayName === "SelectItem"
    );

    const shouldShowSearch = searchable && items.length > 3;

    const filteredChildren = React.Children.map(children, (child: any) => {
      if (!shouldShowSearch) return child;

      if (child?.type?.displayName !== "SelectItem") return child;
