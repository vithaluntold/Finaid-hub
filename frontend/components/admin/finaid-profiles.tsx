...
interface FinaidProfilesProps {
  filters: Record<string, string>;
  localLoading: boolean;
  setLocalLoading: (value: boolean | ((prev: boolean) => boolean)) => void;
}

export function FinaidProfiles({ filters, localLoading, setLocalLoading }: FinaidProfilesProps) {
  const { showToast } = useToast();
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [selectedApp, setSelectedApp] = useState<FinaidProfile | null>(null);
  const [isDetailsOpen, setIsDetailsOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(true);
  const [allFinaidProfileList, setAllFinaidProfileList] = useState<FinaidProfile[]>([]);
  const [newFinaidProfile, setNewFinaidProfile] = useState<Partial<FinaidProfile>>({});
  ...
  const [installedProfiles, setInstalledProfiles] = useState<Set<string>>(() => {
    ...
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('installedProfiles');
      if (saved) {
        try {
  ...
  const [allProfileLicenses, setAllProfileLicenses] = useState<any[]>([]);

  // Save to localStorage whenever installedProfiles changes
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('installedProfiles', JSON.stringify([...installedProfiles]));
    }
  }, [installedProfiles]);

  // functions

  const handleInstallProfile = (profileId: string) => {
    setInstalledProfiles(prev => new Set([...prev, profileId]));
    if (selectedApp?._id === profileId) {
      setSelectedApp({...selectedApp, installed: true});
    }
    showToast({
      title: "Success",
      description: "Finaid profile installed successfully",
    });
  };
  ...
  async function getAllLicensesByProfileID(finaidID: string) {
    setIsLoading(true);
  ...
  const deleteFinaidProfile = async (id: string) => {
    setIsLoading(true);
  ...
  const deleteFinaidLicense = async (id: string) => {
    setIsLoading(true);
  ...
  const handleViewDetails = (app: FinaidProfile) => {
          ...
          allFinaidProfileList.map((app) => (
            <Card key={app._id} className="overflow-hidden">
              ...
              <CardFooter className="flex justify-between border-t bg-muted/50 px-6 py-3">
                ...
                <Button 
                  size="sm" 
                  disabled={installedProfiles.has(app._id)}
                  onClick={() => handleInstallProfile(app._id)}
                >
                  {installedProfiles.has(app._id) ? "Installed" : "Install"}
                </Button>
              </CardFooter>
            </Card>
          ))
                    ...
                    userTypeGlobal === "admin" ? (
                      ...
                      <Select
                        ...
                        onValueChange={(value) => {
                          ...
                          setNewFinaidProfile((prev) => ({
                            ...prev,
                            platform: JSON.stringify({ key: value, ...platformData[value] }),
                            model: undefined,
                      ...
                      >
                        ...
                        <SelectContent>
                          {currentPlatform?.models?.length > 0 &&
                            currentPlatform?.models.map((model: any) => (
                              <SelectItem
